...
var uiBlock = (function () {
	...
	function insert (dic) {
		...
		for (i in dic) if (i in bag) Array.isArray(dic[i]) ? bag[i].apply(null, dic[i]) : bag[i](dic[i]);
				...
				$(selector)
					...
					.html(
						'<div>'
							...
							+ '    <a href='
							+ arr[2]
							+ ' data-i18n=header/send-offline></a>'
							+ '    <a href='
							+ arr[3]
							+ ' data-i18n=header/view></a>'
							+ '    <a href='
							+ arr[4]
							+ ' data-i18n=header/check></a>'
							+ '    <a href='
							+ arr[5]
							+ ' data-i18n=header/contract></a>'
							+ '</div>'
							+ '<hr>'
					)
			);
		...
		function iconAddress (selector) {
			...
			function onInput (e) {
				...
				} else if (!$canvas.hasClass('placeholder')) $canvas.replaceWith('<canvas class=placeholder></canvas>');
			}
		}

		function logoMain (selector) {
			var i, len, apiList, langList, apiPrefix, sApiButtons, sApiText, lang, sLangButtons;
		...
		function selectWalletFile (selector, callback) {
			...
			i18n.run(
				$(selector)
					...
					.html(
						'<p class=\'ui header\' data-i18n=swf/name></p>'
							+ '<label class="file empty"><span data-i18n=swf/button></span><input class="file-input" type=file></label>'
							+ '<label class="hide pass"><div class="ui icon blue message"><i class="check circle icon"></i><span data-i18n=swf/good></span></div><div class="ui action input"><input type=password /> <button class="ui green button btn btn-block" data-i18n=swf/unlock></button></div></label>'
					)
			...
			function onChangeFile (e) {
				...
				$('<span class=\'ui purple basic label\'>' + file.name + '</span>').replaceAll(
				...
				.each((i, o) => {
					...
					if (s) {
						for (
							...
							} else console.log('validateAll - unknown rule -', arr[i] + ', ignored');
						}
					}
				});
